...
server.put('/api/zoos/:id', (req, res) => {
  ...
  if (!zoo.name) {
    ...
    db('zoos').where("id",id).update('name',zoo.name)
    .then(updated => {
      if (updated.length < 1){
...
server.get('/api/bears/:id', (req, res) => {
  const id = req.params.id;
  db.select().table('bears').where("id", id)
  .then(response => {
    if (response.length < 1) {
      res.status(404).json({error: "ID does not exist."})
    } else {
...
server.delete('/api/bears/:id', (req, res) => {
  ...
  .then(response => {
    if (response < 1) {
      ...
      res.status(200).json({message: `Successfully deleted bear with ID${id}`})
    }
  })
  .catch(err => {
    res.status(500).json({error: "Could not delete from db"})
...
server.put('/api/bears/:id', (req, res) => {
  ...
  if (!bear.name) {
    ...
    db('bears').where("id",id).update('name',bear.name)
    .then(updated => {
      if (updated.length < 1){
        res.status(404).json({error: `This ID ${id} does not exist!`})
      } else {
